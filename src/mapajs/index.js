...
class MainMap extends Component {
  ...
  dateConvert = (fromDate) => {
    ...
    const formattedDate = fromDate.getFullYear() + "-" + (fromDate.getMonth() + 1 > 9 ? fromDate.getMonth() + 1 : '0' + fromDate.getMonth() + 1) + "-" + (fromDate.getDate() > 9 ? fromDate.getDate() : '0' + fromDate.getDate()) + " " + (fromDate.getHours() > 9 ? fromDate.getHours() : '0' + fromDate.getHours()) + ":" + (fromDate.getMinutes() > 9 ? fromDate.getMinutes() : '0' + fromDate.getMinutes()) + ":" + (fromDate.getSeconds() > 9 ? fromDate.getSeconds() : '0' + fromDate.getSeconds());
  ...
  handleSaveMarkers = () => {
    ...
    const markerListRef = firebaseDatabase.ref('Marker').push({
  ...
  render() {
    ...
    return (
      ...
      <div>
        ...
        <Button type="button" onClick={this.handleSaveMarkers} type="danger">
          Guardar
        </Button>
        ...
        >
          ...
          <Marker
            title={"Santo Domingo"}
            name={'Santo Domingo'}
            position={{ lat: -0.2230980943885677, lng: -78.51259076329114 }} />
          <Marker />

          <Marker
            title={"Plaza Grande"}
            name={'Plaza Grande'}
            position={{ lat: -0.220101529811345, lng: -78.51183192334038 }} />
          <Marker />

          <Marker
            title={"Alameda"}
            name={'Alameda'}
            position={{ lat: -0.2167093539292637, lng: -78.50516878464863 }} />
          <Marker />

          <Marker
            title={"Asamblea Nacional"}
            name={'Asamblea Nacional'}
            position={{ lat: -0.21329979536304305, lng: -78.49991204517507 }} />
          <Marker />
